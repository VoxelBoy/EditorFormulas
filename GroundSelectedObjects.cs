...
namespace EditorFormulas
{
	public static partial class Formulas {
		...
		public static void GroundSelectedObjects(float distanceFromGround, LayerMask raycastLayerMask)
		{
			...
			{
				Debug.Log("No rigidbodies among selected objects.");
				return;
			}
			foreach(Transform t in selectedSceneObjects)
			{
				var colliders = new List<Collider>();
				//Add the colliders on the same game objet as the rigidbody
				var collidersOnRigidbody = t.GetComponents<Collider>();
				colliders.AddRange(collidersOnRigidbody);
